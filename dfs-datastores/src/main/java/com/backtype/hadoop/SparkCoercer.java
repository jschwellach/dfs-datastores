...
public class SparkCoercer {
	 private static final String FACTIN_ARG = "coercer_stream_factin_arg";
	    ...
	    public static void coerce(String qualSource, String qualDest, int renameMode, PathLister lister, RecordStreamFactory factin, RecordStreamFactory factout, String extensionOnRename, Configuration configuration) throws IOException {
	     ...
	     // we need to add the following two lines to be able to serialize the
			...
			SparkContext sc = SparkContext.getOrCreate(
					conf.setMaster("local").setAppName("Coercer: " + qualSource + " -> " + qualDest));
	    ...
	    public static class CoercerFunction extends AbstractFileCopyFunction {
	        ...
	        public CoercerFunction(FileCopyArgs args, String tmpRoot, RecordStreamFactory factin, RecordStreamFactory factout) {
	        	super(args, tmpRoot);
	        	this.factin = factin;
			...
			@Override
			protected void copyFile(FileSystem fsSource, Path source, FileSystem fsDest, Path target)
					throws IOException {
